...
## 2026-09-25: opaque bilinear blend

Bilinear paste звал `blend_weighted_channel` для каждого channel, а тот на
полной непрозрачности умножает на 1000 и делит на 1000 * 256 * 256, чтобы
обнулить член destination, который ему уже известен как ноль. Замена на
точное `>> 16` плюс решение о blend, вынесенное из пиксельного цикла в const
parameter, дала:

| Сценарий 1080p/4/Grid | До, ms | После, ms | Δ |
|---|---:|---:|---:|
| Bilinear identity | 5.90 | 5.81 | -1.5% |
| Bilinear Fill | 7.86 | 6.84 | -12.9% |
| Bilinear crop+rotate+mirror | 5.55 | 4.64 | -16.5% |
| Bilinear opacity 85% | 6.21 | 6.12 | -1.5% |

Nearest-сценарии в этом прогоне не изменились: их код не тронут, и это
контрольная группа, по которой видно, сколько здесь шум.

Метод: минимум среднего за восемь чередующихся прогонов двух заранее собранных
бинарей, случайный порядок кейсов, seed по номеру раунда. Повторный прогон того
же бинаря на этой машине гуляет на 2-4%, поэтому меньше 5% результатом не
считается. Одна моя более ранняя цифра в этой линии оказалась именно дрейфом, а
не выигрышем. Все отвергнутые варианты в конце документа получены тем же
прогоном.
...
Измеренно отвергнутые варианты композитора, чтобы их не пробовали второй раз:

- То же сужение blend для nearest path (источник входит как `channel * 256 * 256`,
  поэтому весь числитель делится на 65 536, и blend сворачивается в `u32`
  деление на 1000) плюс тот же hoist: медленнее на всех nearest-сценариях,
  +4% при частичной непрозрачности, +3% на plain nearest, +6% на rotated.
- Вынос rotation match из per-pixel `transformed_source_offset` в maps по осям.
  Потолок измерён заменой match на identity-арифметику: от -0.2% до +1.5% на
  unrotated bilinear, то есть ниже уровня шума. Реализация не написана.
- Четыре tap'а bilinear как `u32` вместо двенадцати byte load: +14.1% Fill,
  +10.6% opacity, +6.7% rotated.
- Сам hoist внутри цикла по channel, а не снаружи: +11% на partial opacity.
